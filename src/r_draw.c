...
void R_InitViewBuffer(INT32 width, INT32 height)
{
	...
	for (i = 0; i < width; i++)
		columnofs[i] = (viewwindowx + i) * vid.bpp;
	...
	for (i = 0; i < height; i++)
	{
		ylookup[i] = ylookup1[i] = screen_main + (i+viewwindowy)*vid.width*vid.bpp;
		ylookup2[i] = screen_main + (i+(vid.height>>1))*vid.width*vid.bpp; // for splitscreen
	}
}
