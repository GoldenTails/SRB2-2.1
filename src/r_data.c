...
#ifdef ROTSPRITE
	rotspritepresent = calloc(numsprites, sizeof (*rotspritepresent));
	if (rotspritepresent == NULL) I_Error("%s: Out of memory looking up sprites", "R_PrecacheLevel");
#endif // ROTSPRITE
...
#ifdef ROTSPRITE
			if (((mobj_t *)th)->flags & MF_ROLLANGLE)
				rotspritepresent[((mobj_t *)th)->sprite] = 1;
#endif
		}
...
#ifdef ROTSPRITE
			// Precache sprites
			if (!sf->rotate && !sf->rotcached[0] && rotspritepresent[i])
			{
				R_CacheRotSprite(sf, 0, sf->flip);
				sf->rotcached[0] = true;
			}
#endif
		}
